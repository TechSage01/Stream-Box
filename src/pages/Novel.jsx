...
const NovelPage = () => {
  ...
  const endpoint = useMemo(() => {
    ...
    // Fetch drama/novel-like series
    return `${API_BASE}/discover/tv?api_key=${API_KEY}&with_genres=18&sort_by=${
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (e.name !== "AbortError") setError("Could not load novels.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
          <h1 className="text-2xl md:text-3xl font-bold">Novels</h1>
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {list.map((show) => (
            <article
              key={show.id}
              className="bg-[#111] rounded-lg overflow-hidden border border-white/5 hover:border-red-700/60 transition"
            >
              <img
                src={
                  show.poster_path
