  ...
  const endpoint = useMemo(() => {
    ...
    if (debounced) {
      return `${API_BASE}/search/tv?api_key=${API_KEY}&query=${encodeURIComponent(
        debounced,
      )}&page=${page}&include_adult=false&with_genres=16`;
    }

    // Fetch animated series by genre (16 = Animation)
    return `${API_BASE}/discover/tv?api_key=${API_KEY}&with_genres=16&sort_by=${
      tab === "top_rated" ? "vote_average.desc" : "popularity.desc"
    }&page=${page}`;
  }, [API_KEY, debounced, tab, page]);

  useEffect(() => {
    if (!endpoint) {
      setError("Missing API key in .env (VITE_TMDB_API_KEY)");
      setList([]);
      return;
    }
    ...
    const run = async () => {
      ...
      try {
        ...
        if (e.name !== "AbortError")
          setError("Could not load animated series.");
      } finally {
        setLoading(false);
      }
    };
    ...
    <main className="min-h-screen bg-black text-white">
      <section className="max-w-7xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row gap-4 md:items-center md:justify-between">
          <h1 className="text-2xl md:text-3xl font-bold">Animated Series</h1>
        ...
        {searching && (
          <div className="relative z-10 px-0 py-6">
        ...
        <div className="mt-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          {list.map((show) => (
            <article
              key={show.id}
              onClick={() => navigate(`/movie/${show.id}?type=animation`)}
